...
def test_helper_server_rejects_unknown_assets():
    service = FakeDeleteService()
    server = HelperServer("127.0.0.1", 0, service)
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    try:
        ...
        try:
            ...
            raise AssertionError("expected asset request to fail")
    finally:
        server.shutdown()
        thread.join(timeout=3)
...
def test_helper_server_stores_safe_request_handler(monkeypatch):
    service = FakeDeleteService()
    marker = object()
    captured = {}

    def fake_init(self, server_address, handler_class):
        captured["server_address"] = server_address
        captured["handler_class"] = handler_class
        self.server_address = server_address

    monkeypatch.setattr("http.server.ThreadingHTTPServer.__init__", fake_init)

    server = HelperServer("127.0.0.1", 0, service, request_handler=lambda path, payload: marker if path == "/backend/status" else None)

    assert captured["server_address"] == ("127.0.0.1", 0)
    assert server.request_handler("/backend/status", {}) is marker
