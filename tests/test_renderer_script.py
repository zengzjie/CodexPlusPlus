...
def test_renderer_script_supports_codex_sidebar_thread_attributes():
    ...
    assert "deletedSessionIds" in text
    assert "collapseDeletedRow" in text
    assert "suppressDeletedSessionRows" in text
    assert "deletedRowMarker" in text
...
def test_renderer_script_contains_conversation_timeline_contract():
    ...
    assert "width: 12px;" in text
    assert "pointer-events: auto;" in text
    assert "width: min(260px, calc(100vw - 64px));" in text
    assert "text-align: left;" in text
    assert "white-space: normal;" in text
    assert "overflow-wrap: anywhere;" in text
    assert "word-wrap: break-all;" not in text
    assert "rgba(0, 104, 255, .35)" in text
    assert "rgba(0, 104, 255, 0)" in text
    ...
    assert "timelineQuestionSnapshot" in text
    assert "sameTimelineQuestionSnapshot" in text
    assert "resolveTimelineQuestionTarget" in text
    assert "setActiveTimelineQuestion" in text
    assert "resolveActiveTimelineQuestionNode" in text
    assert "window.__codexConversationTimelineActiveQuestion" in text
    assert "window.__codexConversationTimelineSnapshot" in text
...
def test_renderer_script_detects_user_questions_for_timeline_without_sidebar_scan():
    ...
    assert "normalizeTimelineQuestionNode" in timeline_detection_code
    assert "sortNodesByDocumentOrder" in timeline_detection_code
    ...
    assert "closest('[data-testid=\"conversation-turn\"]')" not in timeline_detection_code
    assert "sortNodesByDocumentOrder(" in timeline_detection_code
...
def test_renderer_script_timeline_uses_stable_hover_and_scroll_behavior():
    ...
    assert 'run("smooth", true)' in text
    assert "scheduleTimelineTargetScroll" in text
    assert "__codexConversationTimelineActivationId" in text
    assert "setActiveTimelineQuestion(index, text, fallbackNode)" in text
    assert 'setTimeout(() => run("auto", false), 80)' in text
    assert 'setTimeout(() => run("auto", false), 220)' in text
    assert "const lockedTargetNode =" in text
    assert "if (!scrollTimelineTarget(lockedTargetNode, behavior)) return" in text
    assert 'requestAnimationFrame(() => run("smooth", true))' in text
    marker_code = text[text.index("function createConversationTimelineMarker"):text.index("\n\n  function refreshConversationTimeline")]
    assert "pointerup" in marker_code
    assert "click" in marker_code
    assert "marker.__codexConversationTimelineLastActivatedAt" in marker_code
    assert "event.type === \"click\"" in marker_code
    assert "resolveActiveTimelineQuestionNode()" in marker_code
    assert "marker.classList.add(\"codex-conversation-timeline-marker-active\")" in marker_code
    assert "scheduleTimelineTargetScroll(markerIndex, question.text, question.node)" in marker_code
...
def test_renderer_script_clears_focus_and_removes_deleted_rows():
    ...
    remove_deleted_code = text[remove_deleted_start:remove_deleted_end]
    ...
    assert "row.querySelectorAll(`.${actionGroupClass}`)" in remove_deleted_code
...
def test_renderer_script_uses_bridge_helper_calls_with_backend_http_fallback():
    text = Path("codex_session_delete/inject/renderer-inject.js").read_text(encoding="utf-8")
    assert "window.__codexSessionDeleteBridge" in text
    assert "async function postBackendJson(path, payload)" in text
    assert "fetch(`${helperBase}${path}`" in text
    assert 'message: "本地服务不可用，请重启启动器"' in text
    ...
    assert "postBackendJson(\"/backend/status\", {})" in text
    assert "postBackendJson(\"/backend/repair\", {})" in text
...
def test_renderer_script_includes_user_script_manager_ui_contract():
    ...
    assert 'message: "本地服务已断开"' in text
    assert 'message: "本地服务不可用，请重启启动器"' in text
    ...
    assert "top: var(--codex-plus-menu-top, 0)" in text
...
def test_renderer_script_omits_sponsor_tab():
    text = Path("codex_session_delete/inject/renderer-inject.js").read_text(encoding="utf-8")
