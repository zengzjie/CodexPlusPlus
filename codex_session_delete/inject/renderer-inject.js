(() => {
  ...
  const baseBtnClass = "codex-base-button";
  ...
  window.__codexProjectMoveRuntimeId =
    (window.__codexProjectMoveRuntimeId || 0) + 1;
  ...
  const selectors = {
    ...
    nativeMenuBar:
      '.flex.items-center.gap-0\\.5, [class*="flex items-center gap-0.5"]',
    archiveNav:
      'button[aria-label="已归档对话"], button[aria-label="Archived conversations"]',
    disabledInstallButton:
      'button:disabled.w-full.justify-center, [role="button"][aria-disabled="true"].cursor-not-allowed',
  ...
  function installStyle() {
    ...
    if (
      existingStyle?.dataset.codexDeleteStyleVersion === codexDeleteStyleVersion
    )
      return;
    ...
    style.textContent = `
      ...
      .codex-plus-tab-button { border: 1px solid rgba(255,255,255,.14); border-radius: 999px; background: transparent; color: #d1d5db; font: 12px system-ui, sans-serif; padding: 5px 10px; cursor: pointer; }
      ...
      .codex-plus-backend-repair { border: 1px solid rgba(255,255,255,.18); border-radius: 7px; background: #3f3f46; color: #f3f4f6; font: 12px system-ui, sans-serif; padding: 6px 8px; cursor: pointer; }
      ...
      .codex-plus-user-script-reload { border: 1px solid rgba(255,255,255,.18); border-radius: 7px; background: #3f3f46; color: #f3f4f6; font: 12px system-ui, sans-serif; padding: 6px 8px; cursor: pointer; }
      .${timelineClass} {
        position: fixed;
        top: calc(72px + 12px);
        right: 12px;
      ...
      @keyframes codex-conversation-timeline-pulse {
        0% { box-shadow: 0 0 0 0 rgba(0, 104, 255, .35); }
        100% { box-shadow: 0 0 0 6px rgba(0, 104, 255, 0); }
      }
    `;
  ...
  function refreshDeletedSessionStyle() {
    ...
    const styleSelectors = Array.from(deletedSessionIds)
      .map(
        (sessionId) =>
  ...
  function codexPlusSettings() {
    try {
      return {
        ...defaultCodexPlusSettings(),
        ...JSON.parse(localStorage.getItem(codexPlusSettingsKey) || "{}"),
      };
    } catch {
  ...
  function renderCodexPlusMenu() {
    document
      .querySelectorAll(".codex-plus-toggle[data-codex-plus-setting]")
      .forEach((button) => {
  ...
  function refreshCodexPlusBackendToggles() {
    document
      .querySelectorAll(".codex-plus-toggle[data-codex-backend-setting]")
      .forEach((button) => {
  ...
  let codexPlusUserScripts = {
    enabled: true,
    builtin_dir: "",
    user_dir: "",
    scripts: [],
  };
  let codexPlusBackendStatus = {
    status: "checking",
    message: "正在检查本地服务…",
  };
  ...
  function renderBackendStatus() {
    ...
    const fallbackMessage =
      status === "ok"
        ...
        : status === "checking"
          ? "正在检查本地服务"
          : "本地服务已断开";
    const statusMessage = codexPlusBackendStatus.message || fallbackMessage;
    ...
    document
      .querySelectorAll("[data-codex-backend-indicator]")
      .forEach((indicator) => {
  ...
  function withBackendTimeout(request) {
    return Promise.race([
      ...
      new Promise((resolve) =>
        setTimeout(
          () => resolve({ status: "failed", message: "本地服务已断开" }),
          2000,
        ),
      ),
    ]);
  ...
  async function postBackendJson(path, payload) {
    const bridgeResult = await withBackendTimeout(postJson(path, payload));
    if (bridgeResult?.status !== "failed") return bridgeResult;
    try {
      ...
      if (!response.ok) {
        return { status: "failed", message: "本地服务不可用，请重启启动器" };
      }
  ...
  async function checkBackendStatus() {
    codexPlusBackendStatus = await postBackendJson("/backend/status", {});
    renderBackendStatus();
  }
  ...
  async function repairBackend() {
    ...
    renderBackendStatus();
    try {
      codexPlusBackendStatus = await postBackendJson("/backend/repair", {});
    } catch (error) {
      codexPlusBackendStatus = {
        status: "failed",
        message: "本地服务修复失败",
  ...
  function renderUserScripts() {
    const enabledToggle = document.querySelector(
      "[data-codex-user-scripts-enabled]",
    );
    if (enabledToggle)
    ...
    if (dirs)
      dirs.textContent = `内置：${codexPlusUserScripts.builtin_dir || "未找到"}  用户：${codexPlusUserScripts.user_dir || "未找到"}`;
    ...
    list.innerHTML = codexPlusUserScripts.scripts
      .map(
        (script) => `
  ...
  function selectCodexPlusTab(tab) {
    ...
    document.querySelectorAll("[data-codex-plus-tab]").forEach((button) => {
      button.dataset.active = String(
        button.getAttribute("data-codex-plus-tab") === tab,
      );
    });
  ...
  function openCodexPlusModal() {
    document
      .querySelectorAll(".codex-plus-modal-overlay")
      .forEach((node) => node.remove());
    document
      .querySelectorAll('[data-codex-plus-dialog="true"]')
      .forEach((node) => node.remove());
    ...
    overlay.innerHTML = `
      <div class="codex-plus-modal-content" role="dialog" aria-modal="true" aria-label="Codex++">
        <div class="codex-plus-modal-header">
          <div class="codex-plus-modal-title"><span class="codex-plus-backend-indicator" data-codex-backend-indicator="true" data-status="checking"></span><span>Codex++</span></div>
        ...
        <div class="codex-plus-modal-body">
          <div class="codex-plus-panel" data-codex-plus-panel="home">
            <div class="codex-plus-row">
              <div><div class="codex-plus-row-title">本地服务连接</div><div class="codex-plus-row-description">每 5 秒检查一次 launcher 本地服务状态；断开时可尝试修复本地服务运行。</div></div>
              <div class="codex-plus-backend-status">
                <div class="codex-plus-backend-label" data-codex-backend-status="true" data-status="checking">正在检查本地服务…</div>
                <button type="button" class="codex-plus-backend-repair" data-codex-backend-repair="true" hidden>修复本地服务</button>
              </div>
            </div>
            <div class="codex-plus-row">
    ...
    overlay.addEventListener(
      ...
      (event) => {
        ...
        if (
          ...
          target?.closest(".codex-plus-modal-close")
        ) {
          overlay.remove();
          return;
        }
        ...
        const userScriptsEnabled = target?.closest(
          "[data-codex-user-scripts-enabled]",
        );
        if (userScriptsEnabled) {
          loadUserScripts("/user-scripts/set-enabled", {
            enabled: userScriptsEnabled.dataset.enabled !== "true",
          });
          return;
        }
        const userScriptToggle = target?.closest(
          "[data-codex-user-script-key]",
        );
        if (userScriptToggle) {
          loadUserScripts("/user-scripts/set-script-enabled", {
            key: userScriptToggle.getAttribute("data-codex-user-script-key"),
            enabled: userScriptToggle.dataset.enabled !== "true",
          });
          return;
        }
  ...
  function removeDuplicateCodexPlusMenus(keep) {
    document
      .querySelectorAll(`#${codexPlusMenuId}, [data-codex-plus-menu="true"]`)
      .forEach((node) => {
        if (node !== keep) node.remove();
      });
    Array.from(document.querySelectorAll("button")).forEach((button) => {
      if (
        (button.textContent || "").trim() === `Codex++ ${codexPlusVersion}` &&
        !button.closest(`#${codexPlusMenuId}`)
      ) {
        button.remove();
      }
    });
  ...
  function headerTitleRegion(header) {
    const candidates = Array.from(
      header?.querySelectorAll?.(
        '[data-state], [class*="truncate"], [class*="text-base"]',
      ) || [],
    );
    return (
      candidates.find((node) => {
        ...
        return (
          node.closest?.(".draggable") ||
          node.closest?.('[class*="grid-cols-[minmax(0,1fr)]"]')
        );
      }) || null
    );
  }
  ...
  function updateFloatingCodexPlusMenuPosition(menu) {
    ...
    if (anchor) {
      const measuredGap = toolbarButtons[1]
        ? toolbarButtons[1].rect.left - toolbarButtons[0].rect.right
        : 0;
      const styles = anchor.button.parentElement
        ? getComputedStyle(anchor.button.parentElement)
        : null;
      const gap = Math.max(
        numericCssValue(styles?.columnGap || styles?.gap),
        measuredGap,
        0,
      );
      setCssPropIfChanged(
      ...
      setCssPropIfChanged(
        ...
        `${anchor.rect.height}px`,
      );
      setCssPropIfChanged(
        menu,
        "--codex-plus-menu-right",
        `${Math.max(0, window.innerWidth - anchor.rect.left + gap)}px`,
      );
      return;
    }
  ...
  function installCodexPlusMenu() {
    ...
    if (existing && existing.dataset.codexPlusMenuVersion !== "6") {
      ...
      existing &&
      insertionPoint &&
      existing.parentElement === insertionPoint.parent
    ) {
      configureCodexPlusTrigger(
        existing,
        existing.querySelector("button"),
        insertionPoint.nativeButtonClass,
      );
      removeDuplicateCodexPlusMenus(existing);
      return;
    }
    ...
    if (insertionPoint) {
      menu.className = "";
      const safeBefore =
        insertionPoint.before?.parentElement === insertionPoint.parent
  ...
  function pluginEntryButton() {
    const byIcon = document
      .querySelector(`${selectors.pluginNavButton} ${selectors.pluginSvgPath}`)
      ?.closest("button");
    if (byIcon) return byIcon;
    return (
      Array.from(document.querySelectorAll(selectors.pluginNavButton)).find(
        (button) =>
          /^(插件|Plugins)(\s+-\s+.*)?$/i.test(
  ...
  function labelUnlockedPluginEntry(button) {
    const labelTextNode = Array.from(button.querySelectorAll("span, div"))
      ...
      .find(
        (node) =>
          node.nodeType === 3 &&
          /^(插件|Plugins)( - 已解锁| - Unlocked)?$/i.test(
            (node.nodeValue || "").trim(),
          ),
      );
  ...
  function enablePluginEntry() {
    ...
    const reactPropsKey = Object.keys(pluginButton).find((key) =>
  ...
  function pluginInstallCandidates() {
    return Array.from(
      document.querySelectorAll(selectors.disabledInstallButton),
    );
  }
  ...
  function labelForcedInstallButton(button) {
    const textNode = Array.from(button.childNodes).find(
      (node) =>
        node.nodeType === 3 &&
        (/^安装\s/.test((node.nodeValue || "").trim()) ||
          /^Install\s/.test((node.nodeValue || "").trim()) ||
          (node.nodeValue || "").trim() === "强制安装"),
    );
    if (textNode) {
  ...
  function suppressDeletedSessionRows() {
    document.querySelectorAll(selectors.sidebarThread).forEach((row) => {
      if (
  ...
  function archivePageHintVisible() {
    ...
    if (
      document.querySelector(
        '[data-codex-archive-page-row="true"], [data-codex-archive-delete-all]',
      )
    )
      return true;
    const archiveNav = document.querySelector(selectors.archiveNav);
    if (archiveNav?.className?.includes?.("bg-token-list-hover-background"))
      return true;
    return !!Array.from(document.querySelectorAll("h1, h2, h3")).find(
      (element) => (element.textContent || "").trim() === "已归档对话",
    );
  }

  function archiveRowFromUnarchiveButton(button) {
    return (
      button.closest('[data-codex-archive-page-row="true"]') ||
  ...
  function archivedPageRows() {
    ...
    const rows = Array.from(document.querySelectorAll("button"))
      .filter((button) => (button.textContent || "").trim() === "取消归档")
  ...
  function archivedSessionRows() {
    ...
    return sessionRows().filter(
      (row) =>
        row.querySelector('button[aria-label="取消归档对话"]') ||
        row.outerHTML.includes("取消归档") ||
        row.outerHTML.includes("unarchive"),
    );
  }
  ...
  function sessionRefFromRow(row) {
    ...
    const titleNode = row.querySelector(
      `${selectors.threadTitle}, .truncate.select-none, .truncate.text-base`,
    );
    const rawTitle =
      titleNode?.textContent ||
      (titleNode ? "" : row.textContent || "Untitled session");
    const title = (
      titleNode
        ? rawTitle
        : rawTitle.replace(
            /\s*(导出|删除|移动|移出项目)(\s*(导出|删除|移动|移出项目))*$/g,
            "",
          )
    )
      .trim()
  ...
  async function codexStateApi() {
    codexStateApiPromise =
      codexStateApiPromise || import("./assets/vscode-api-Dc9pX2Bc.js");
  ...
  async function getCodexGlobalState(key) {
    const result = await codexStateCall("get-global-state", {
      params: { key },
    });
  ...
  function projectRowListItem(projectRow) {
    return (
      projectRow.closest?.('[role="listitem"][aria-label]') ||
      projectRow.closest?.('[role="listitem"]') ||
      projectRow
    );
  }
  ...
  function serializableProjectTarget(target) {
    return {
      ...
      path: target.path,
      normalizedPath:
        target.normalizedPath || normalizeWorkspacePath(target.path),
    };
  }
  ...
  function readLegacyProjectMoveProjection() {
    try {
      const parsed = JSON.parse(
        localStorage.getItem(legacyProjectMoveOverridesKey) || "{}",
      );
      ...
      for (const [key, value] of Object.entries(parsed)) {
        ...
        next[sessionId] = {
          ...
          targetLabel: String(
            value.targetLabel || displayProjectName(value.targetCwd),
          ),
          title: String(value.title || ""),
          sortMs: sortMsForSession(
            sessionId,
            value.sortMs || value.updatedAtMs || value.updated_at_ms,
          ),
          sortMsTrusted: false,
  ...
  function readProjectMoveProjection() {
    try {
      const parsed = JSON.parse(
        localStorage.getItem(projectMoveProjectionKey) || "{}",
      );
      const raw =
      ...
      for (const [key, value] of Object.entries(merged)) {
        ...
        if (
          ...
          now - value.at > projectMoveProjectionTtlMs
        )
          continue;
        const targetKind =
          value.targetKind === "projectless" ? "projectless" : "project";
        ...
        projection[sessionId] = {
          ...
          targetLabel: String(
            value.targetLabel ||
              value.label ||
              (targetKind === "projectless"
                ? "普通对话"
                : displayProjectName(targetCwd)),
          ),
          title: String(value.title || ""),
          sortMs: sortMsForSession(
            sessionId,
  ...
  function writeProjectMoveProjection(projection) {
    try {
      localStorage.setItem(
        projectMoveProjectionKey,
        JSON.stringify(projection || {}),
      );
      localStorage.removeItem(legacyProjectMoveOverridesKey);
    } catch (error) {
      window.__codexProjectMoveProjectionFailures =
        window.__codexProjectMoveProjectionFailures || [];
      window.__codexProjectMoveProjectionFailures.push(
  ...
  function saveProjectMoveProjection(ref, target, sortMs) {
    ...
    projection[id] = {
      ...
      targetLabel:
        target.label ||
        (target.kind === "projectless"
          ? "普通对话"
          : displayProjectName(target.path)),
      title: ref.title || "",
  ...
  function clearProjectMoveProjection(ref) {
    const projection = readProjectMoveProjection();
    const keys = threadIdVariants(ref.session_id)
      .map(projectMoveSessionKey)
      .filter(Boolean);
    let changed = false;
    keys.forEach((key) => {
  ...
  function projectionForSessionId(
    sessionId,
    projection = readProjectMoveProjection(),
  ) {
  ...
  function projectRowFromListItem(projectItem) {
    ...
    if (projectItem.matches?.("[data-app-action-sidebar-project-row]"))
      return projectItem;
    return (
      projectItem.querySelector?.("[data-app-action-sidebar-project-row]") ||
      null
    );
  }
  ...
  function targetLabel(target) {
    return (
      target?.label ||
      target?.targetLabel ||
      displayProjectName(targetPath(target))
    );
  }
  ...
  function projectItemMatchesTarget(projectItem, target) {
    ...
    if (projectPath && sameWorkspacePath(projectPath, targetPath(target)))
      return true;
    const actual = normalizeProjectLabel(
  ...
  function findProjectListItem(target) {
    const nativeTarget = nativeProjectTargets().find((project) =>
      sameWorkspacePath(project.path, targetPath(target)),
    );
    ...
    return (
      Array.from(
        section.querySelectorAll('[role="listitem"][aria-label]'),
      ).find((item) => projectItemMatchesTarget(item, target)) || null
    );
  }
  ...
  function chatsThreadList() {
    return (
      chatsSection()?.querySelector?.(
        '[role="list"][aria-label="对话"], [role="list"]',
      ) || null
    );
  }
  ...
  function rowIsUnderTarget(row, target) {
    return target?.targetKind === "projectless" ||
      target?.kind === "projectless"
      ? rowIsInChats(row)
      : rowIsUnderTargetProject(row, target);
  }
  ...
  function isRelativeTimeText(text) {
    const value = String(text || "")
      .replace(/\s+/g, " ")
      .trim();
    return /^(刚刚|just now|\d+\s*(秒|秒钟|分|分钟|小时|天|日|周|星期|个月|月|年|sec|secs|second|seconds|min|mins|minute|minutes|h|hr|hrs|hour|hours|d|day|days|w|wk|wks|week|weeks|mo|mos|month|months|y|yr|yrs|year|years))$/i.test(
      value,
    );
  }
  ...
  function closestTimeWrapper(row, node) {
    ...
    while (current && current !== root && current !== row) {
      ...
      if (
        current.dataset?.codexProjectMoveTimeWrapper === "true" ||
        (className.includes("ml-[3px]") && className.includes("min-w-[26px]"))
      )
        return current;
  ...
  function nodeInsideStatusIcon(row, node) {
    ...
    while (current && current !== stop && current !== row) {
      ...
      if (className.includes("size-5") && className.includes("shrink-0"))
        return true;
      if (
        className.includes("contain-paint") &&
        className.includes("contain-layout")
      )
        return true;
      current = current.parentElement;
    }
  ...
  function cleanupManagedStatusIconTimeNodes(row) {
    Array.from(
      row?.querySelectorAll?.('[data-codex-project-move-time="true"]') || [],
    ).forEach((node) => {
      ...
      if (node.children.length === 0 && isRelativeTimeText(text))
        node.textContent = "";
    });
  }
  ...
  function nodeLooksLikeTimeLabel(row, node) {
    ...
    if (
      (className.includes("tabular-nums") ||
        className.includes("text-token-description-foreground")) &&
      text.length <= 24
    )
      return true;
    ...
    if (!rowRect || !nodeRect || rowRect.width <= 0 || nodeRect.width <= 0)
      return false;
    return (
      nodeRect.left >= rowRect.left + rowRect.width * 0.45 ||
      nodeRect.right >= rowRect.right - 96
    );
  }
  ...
  function rowTimeLabelCandidates(row) {
    ...
    const raw = Array.from(
      root?.querySelectorAll?.("div, span, time, small") || [],
    ).filter((node) => {
      if (nodeIsThreadTitle(row, node)) return false;
      return nodeLooksLikeTimeLabel(row, node);
    });
    return raw.filter(
      (node) => !raw.some((other) => other !== node && node.contains(other)),
    );
  }
  ...
  function rowTimeLabelNode(row) {
    ...
    return (
      candidates.find(
        (node) =>
          node.dataset?.codexProjectMoveTime !== "true" &&
          !node.closest?.('[data-codex-project-move-time-wrapper="true"]'),
      ) ||
      candidates[0] ||
      null
    );
  ...
  function ensureRowTimeLabelNode(row) {
    ...
    wrapper.className =
      "ml-[3px] flex items-center justify-end gap-1 min-w-[26px]";
    wrapper.dataset.codexProjectMoveTimeWrapper = "true";
    const inner = document.createElement("div");
    const label = document.createElement("div");
    label.className =
      "text-token-description-foreground text-sm leading-4 empty:hidden tabular-nums overflow-visible truncate text-right group-focus-within:opacity-0 group-hover:opacity-0";
  ...
  function rowProjectionKind(row) {
    return (
      row?.dataset?.codexProjectMoveTargetKind ||
      rowListItem(row)?.dataset?.codexProjectMoveTargetKind ||
      ""
    );
  }

  function rowSortMs(row, ref = sessionRefFromRow(row), target = null) {
    return sortMsForSession(
      ref.session_id,
      target?.sortMs ||
        row?.dataset?.codexProjectMoveSortMs ||
        rowListItem(row)?.dataset?.codexProjectMoveSortMs,
    );
  }
  ...
  function insertRowItemByTime(list, item, row, target) {
    ...
    for (const child of existingItems) {
      ...
      if (
        sortMs > childSortMs ||
        (sortMs === childSortMs && sessionKey > childKey)
      ) {
        list.insertBefore(item, child);
        return;
      }
  ...
  function projectMoveInjectedList(projectItem) {
    let list = projectItem.querySelector(
      '[data-codex-project-move-injected-list="true"]',
    );
    if (!list) {
      const body =
        Array.from(projectItem.children).find((child) =>
  ...
  function projectThreadList(projectItem, target) {
    ...
    const projectLists = Array.from(
      projectItem.querySelectorAll("[data-app-action-sidebar-project-list-id]"),
    );
    return (
      projectLists.find((list) =>
        sameWorkspacePath(
          list.getAttribute("data-app-action-sidebar-project-list-id"),
          targetCwd,
        ),
      ) ||
      projectLists[0] ||
  ...
  function projectEmptyStateNodes(projectItem) {
    ...
    return Array.from(projectItem.querySelectorAll("div, span")).filter(
      (node) => {
        if (node.classList?.contains("overflow-hidden")) return false;
        if (
          node.closest(
            '[data-app-action-sidebar-thread-id], [data-codex-project-move-injected-list="true"]',
          )
        )
          return false;
  ...
  function updateProjectMoveEmptyStates() {
    document
      .querySelectorAll('[data-codex-project-move-injected-list="true"]')
      .forEach((list) => {
        const projectItem = list.closest('[role="listitem"][aria-label]');
        const hasRows = Array.from(list.children).some(
          (child) =>
            child.querySelector?.("[data-app-action-sidebar-thread-id]") ||
            child.matches?.("[data-app-action-sidebar-thread-id]"),
        );
        if (!hasRows) list.remove();
        if (projectItem) setProjectEmptyStateHidden(projectItem, hasRows);
      });
    document
      .querySelectorAll('[data-codex-project-move-empty-hidden="true"]')
      .forEach((node) => {
        const projectItem = node.closest('[role="listitem"][aria-label]');
        const list = projectItem?.querySelector?.(
          '[data-codex-project-move-injected-list="true"]',
        );
    ...
    rows.forEach((row) => {
      ...
      if (target && rowIsUnderTarget(row, target)) {
        ...
        if (existingRow && existingRow !== row) {
          ...
          const rowToRemove =
            existingIsProjection && !currentIsProjection ? existingRow : row;
        ...
        if (
          !hadProjectionKind &&
          typeof target.at === "number" &&
          now - target.at > projectMoveProjectionSettleMs
        )
          settledRefs.push(ref);
        const moved =
          target.targetKind === "projectless"
            ? moveRowToChats(row, target)
            : moveRowToProjectList(row, target);
  ...
  function scheduleProjectMoveProjection() {
    if (
      ...
      window.__codexProjectMoveProjectionTimer
    )
      return;
    window.__codexProjectMoveProjectionTimer = setTimeout(() => {
      if (window.__codexProjectMoveRuntimeId !== codexProjectMoveRuntimeId)
        return;
      window.__codexProjectMoveProjectionTimer = null;
  ...
  async function refreshRecentConversationsForHost() {
    try {
      const signals =
        await import("./assets/app-server-manager-signals-C1h8B-R-.js");
      if (typeof signals.rn === "function")
        await signals.rn("refresh-recent-conversations-for-host", {
          hostId: "local",
          sortKey: "updated_at",
        });
    } catch (error) {
      window.__codexProjectMoveRefreshFailures =
        window.__codexProjectMoveRefreshFailures || [];
      window.__codexProjectMoveRefreshFailures.push(
        String(error?.stack || error),
      );
    }
  }
  ...
  function chatsSortNeedsCorrection(rows) {
    ...
    for (const row of rows) {
      ...
      if (
        sortMs > previousSortMs ||
        (sortMs === previousSortMs && key > previousKey)
      )
        return true;
      previousSortMs = sortMs;
      previousKey = key;
  ...
  function reorderChatsRows(rows) {
    ...
    const firstNonThreadItem = Array.from(list.children).find(
      (child) => !rowItems.has(child) && !threadRowFromListItem(child),
    );
    const orderedRows = [...rows].sort((left, right) => {
      ...
      return projectMoveSessionKey(rightRef.session_id).localeCompare(
        projectMoveSessionKey(leftRef.session_id),
      );
    });
    orderedRows.forEach((row) =>
      list.insertBefore(rowListItem(row), firstNonThreadItem || null),
    );
    cachedSessionRowsAt = 0;
  }
  ...
  async function applyChatsSortCorrection() {
    ...
    const signature = refs
      .map((ref) => projectMoveSessionKey(ref.session_id))
      .join("|");
    const allRowsHaveSortMs = rows.every((row) =>
      numericTimestamp(
        row.dataset.codexProjectMoveSortMs ||
          rowListItem(row).dataset.codexProjectMoveSortMs,
      ),
    );
    const shouldRefreshSortKeys =
      signature !== chatsSortSignature ||
      !allRowsHaveSortMs ||
      Date.now() - chatsSortLastFetchAt > chatsSortDbRefreshIntervalMs;
    if (!shouldRefreshSortKeys && !chatsSortNeedsCorrection(rows)) return;
    chatsSortInFlight = true;
    try {
      if (shouldRefreshSortKeys) {
        const result = await postJson("/thread-sort-keys", {
          sessions: refs,
      ...
      chatsSortSignature = visibleChatsRows()
  ...
  function scheduleChatsSortCorrection(delay = chatsSortRefreshIntervalMs) {
    if (
      !codexPlusSettings().projectMove ||
      window.__codexProjectMoveChatsSortTimer
    )
      return;
    window.__codexProjectMoveChatsSortTimer = setTimeout(() => {
      if (window.__codexProjectMoveRuntimeId !== codexProjectMoveRuntimeId)
        return;
      window.__codexProjectMoveChatsSortTimer = null;
      applyChatsSortCorrection()
        .catch((error) => {
          window.__codexProjectMoveSortFailures =
            window.__codexProjectMoveSortFailures || [];
          window.__codexProjectMoveSortFailures.push(
            String(error?.stack || error),
          );
        })
        .finally(() => {
  ...
  async function setProjectlessThreadIds(ref, mode) {
    ...
    const existingIds = await getCodexGlobalState(
    ...
    const nextIds =
      mode === "add"
        ? uniqueValues([...ids, ...variants])
        : ids.filter((id) => !variantSet.has(id));
    if (
      nextIds.length !== ids.length ||
      nextIds.some((id, index) => id !== ids[index])
    )
      await setCodexGlobalState("projectless-thread-ids", nextIds);
  }

  async function clearThreadWorkspaceHints(ref) {
    const variants = threadIdVariants(ref.session_id);
    if (variants.length === 0) return;
    const hints = objectGlobalState(
      await getCodexGlobalState("thread-workspace-root-hints").catch(
        () => ({}),
      ),
    );
    const hintKeys = variants.filter((id) =>
  ...
  function isNativeProjectTarget(target) {
    return (
      target?.kind === "project" &&
      nativeProjectTargets().some((project) =>
        sameWorkspacePath(project.path, target.path),
      )
    );
  }
  ...
  async function moveSessionToProject(ref, target) {
    ...
    if (!isNativeProjectTarget(target))
      throw new Error("目标项目不在 Codex 项目列表中");
    const result = await postJson("/move-thread-workspace", {
      ...ref,
      target_cwd: target.path,
    });
    if (result.status !== "moved")
      throw new Error(result.message || "移动项目失败");
  ...
  function showToast(message, undoToken) {
    document
      .querySelectorAll(".codex-delete-toast")
      .forEach((node) => node.remove());
  ...
  function confirmDelete(title) {
    document
      .querySelectorAll(".codex-delete-confirm-overlay")
      .forEach((node) => node.remove());
    return new Promise((resolve) => {
      ...
      overlay.addEventListener(
        ...
        (event) => {
          if (
            event.target === overlay ||
            event.target.closest("[data-codex-delete-cancel]")
          ) {
            finish(false, event);
            return;
          }
  ...
  function removeDeletedRow(row, button, ref) {
    ...
    row
      .querySelectorAll(`.${actionGroupClass}`)
      .forEach((node) => node.remove());
    cachedSessionRows = cachedSessionRows.filter(
      (cachedRow) => cachedRow !== row,
    );
    if (shouldReload) {
  ...
  function updateDeleteButtonOffsets() {
    sessionRows().forEach((row) => {
      const hasArchiveConfirm = Array.from(row.querySelectorAll("button")).some(
        (button) => {
          ...
          if (
            ...
            button.classList.contains(exportButtonClass) ||
            label === "归档对话" ||
            label === "置顶对话"
          )
            return false;
          return (
            ...
            (text.length > 0 &&
              rect.width > 0 &&
              rect.width <= 36 &&
              rect.x > row.getBoundingClientRect().right - 50)
          );
        },
      );
  ...
  function sortStateFromMoveResult(result, ref, row) {
    ...
    return {
      sortMs: trustedSortMs || rowSortMs(row, ref),
      sortMsTrusted: !!trustedSortMs,
    };
  }
  ...
  function finishProjectMove(row, button, ref, target, message) {
    ...
    saveProjectMoveProjection(
      ref,
      target,
      target.sortMs || rowSortMs(row, ref, target),
    );
    ...
    try {
      if (target.kind === "projectless") {
        ...
        finishProjectMove(
          ...
          button,
          ref,
          { ...target, ...sortStateFromMoveResult(result, ref, row) },
          `已移动到普通对话：“${ref.title || ref.session_id}”`,
        );
      } else {
        ...
        finishProjectMove(
          ...
          ref,
          { ...target, ...sortStateFromMoveResult(result, ref, row) },
          `已移动到“${target.label}”：“${ref.title || ref.session_id}”`,
        );
      }
    } catch (error) {
      button.disabled = false;
  ...
  async function openProjectMoveMenuForRow(row, button, ref, event) {
    ...
    releaseDeleteFocus(row, button);
    document
      .querySelectorAll(`.${projectMoveOverlayClass}`)
      .forEach((node) => node.remove());
    const overlay = document.createElement("div");
  ...
  function installDeleteButtonEventDelegation() {
    document.removeEventListener(
      "pointerup",
      window.__codexSessionDeleteDocumentDeleteHandler,
      true,
    );
    document.removeEventListener(
      "click",
    ...
    ["pointerdown", "mousedown", "mouseup", "touchstart"].forEach(
      (eventName) => {
        button.addEventListener(
          eventName,
          (event) => stopActionButtonEvent(row, button, event),
          true,
        );
      },
    );
  ...
  function attachButton(row) {
    ...
    const existingDeleteButton = existingGroup?.querySelector(
      `.${buttonClass}`,
    );
    const existingExportButton = existingGroup?.querySelector(
      `.${exportButtonClass}`,
    );
    const existingMoveButton = existingGroup?.querySelector(
      `.${projectMoveButtonClass}`,
    );
    const hasUnexpectedDelete =
      !settings.sessionDelete && !!existingDeleteButton;
    const hasUnexpectedExport =
      !settings.markdownExport && !!existingExportButton;
    ...
    const deleteReady =
      !settings.sessionDelete ||
      existingDeleteButton?.dataset.codexDeleteVersion === codexDeleteVersion;
    const exportReady =
      !settings.markdownExport ||
      existingExportButton?.dataset.codexExportVersion === codexExportVersion;
    const moveReady =
      !settings.projectMove ||
      existingMoveButton?.dataset.codexProjectMoveVersion ===
        codexProjectMoveVersion;
    const groupReady =
      existingGroup?.dataset.codexActionGroupVersion ===
      codexActionGroupVersion;
    if (
      groupReady &&
      deleteReady &&
      exportReady &&
    ...
    if (settings.projectMove) {
      ...
      const openProjectMove = (event) =>
        openProjectMoveMenuForRow(row, moveButton, ref, event);
      installActionButtonEvents(row, moveButton, openProjectMove);
      group.appendChild(moveButton);
      setTimeout(
        () => refreshActionButton(moveButton, row, openProjectMove),
        0,
      );
    }
    ...
    if (settings.sessionDelete) {
      ...
      const openDeleteConfirm = (event) =>
        openDeleteConfirmForRow(row, deleteButton, ref, event);
      installActionButtonEvents(row, deleteButton, openDeleteConfirm);
      group.appendChild(deleteButton);
      setTimeout(
        () => refreshActionButton(deleteButton, row, openDeleteConfirm),
        0,
      );
    }
  ...
  function tryAttachButton(row) {
    try {
      ...
      window.__codexSessionDeleteAttachButtonFailures =
        window.__codexSessionDeleteAttachButtonFailures || [];
      window.__codexSessionDeleteAttachButtonFailures.push(
        String(error?.stack || error),
      );
    }
  }
  ...
  function archivedRefFromRow(row) {
    ...
    if (archivedThread?.id || archivedThread?.sessionId) {
      return {
        session_id: archivedThread.id || archivedThread.sessionId,
        title:
          archivedThread.title ||
          row.querySelector(".truncate.text-base")?.textContent?.trim() ||
          "Untitled session",
      };
    }
  ...
  function archiveTitleContainer() {
    const heading = Array.from(document.querySelectorAll("h1, h2, h3")).find(
      (element) => isArchiveTitleText((element.textContent || "").trim()),
    );
    if (heading) return heading;
    return Array.from(document.querySelectorAll("h1, h2, h3, div, span")).find(
      (element) =>
        isArchiveTitleText((element.textContent || "").trim()) &&
        element.getBoundingClientRect().x > 350,
    );
  }
  ...
  function attachArchivedPageDeleteButton(row) {
    const settings = codexPlusSettings();
    row
      .querySelectorAll("[data-codex-archive-row-action]")
      .forEach((button) => button.remove());
    row.dataset.codexArchiveDeleteRow = "false";
    if (!settings.sessionDelete && !settings.markdownExport) return;
    const unarchiveButton = Array.from(row.querySelectorAll("button")).find(
      (button) => (button.textContent || "").trim() === "取消归档",
    );
  ...
  function installArchivedDeleteAllButton() {
    ...
    if (
      existingButton?.dataset.codexArchiveDeleteAllVersion ===
      codexArchiveDeleteAllVersion
    )
      return;
    existingButton?.remove();
    ...
    const openArchivedDeleteAllConfirm = async (event) => {
      ...
      if (!(await confirmDelete(`全部 ${currentRows.length} 个归档会话`)))
        return;
  ...
  function conversationTimelineQuestionCandidates(root) {
    ...
    const codexUserBubbles = Array.from(
      root.querySelectorAll(
        ".group.flex.w-full.flex-col.items-end.justify-end.gap-1",
      ),
    ).flatMap((group) => {
      return Array.from(group.children).filter((child) =>
        String(child.className || "").includes("bg-token-foreground/5"),
      );
    });
    return [...explicitCandidates, ...codexUserBubbles];
  }

  function normalizeTimelineQuestionNode(node) {
    if (!node?.matches) return null;
    const bubble =
      node.matches('[data-message-author-role="user"]') ||
      String(node.className || "").includes("bg-token-foreground/5")
        ? node
        : node.querySelector(
            '[data-message-author-role="user"], .bg-token-foreground\\/5',
          );
    return bubble || node;
  }
  ...
  function extractTimelineQuestionText(node) {
    ...
    clone
      .querySelectorAll("button, svg, [aria-hidden='true'], .sr-only")
  ...
  function conversationTimelineQuestions() {
    ...
    if (!root?.matches?.('.thread-scroll-container, main, [role="main"]'))
      return [];
    const seen = new Set();
    const candidates = sortNodesByDocumentOrder(
      conversationTimelineQuestionCandidates(root)
        .map(normalizeTimelineQuestionNode)
        .filter(Boolean),
    );
  ...
  function timelineQuestionSnapshot(questions) {
  ...
  function sameTimelineQuestionSnapshot(left, right) {
    if (!Array.isArray(left) || !Array.isArray(right)) return false;
    if (left.length !== right.length) return false;
    return left.every(
      (item, index) =>
        item?.node === right[index]?.node && item?.text === right[index]?.text,
    );
  }

  function resolveTimelineQuestionTarget(index, text, fallbackNode) {
    if (fallbackNode?.isConnected) return fallbackNode;
    const liveQuestions = conversationTimelineQuestions();
    if (liveQuestions.length === 0) return null;
    const direct = liveQuestions[index];
    if (direct?.text === text) return direct.node;
    const exactMatches = liveQuestions
      .map((question, liveIndex) => ({ question, liveIndex }))
      .filter((entry) => entry.question.text === text);
    if (exactMatches.length === 1) return exactMatches[0].question.node;
    if (exactMatches.length > 1) {
      return (
        exactMatches.reduce((best, entry) => {
          if (!best) return entry;
          return Math.abs(entry.liveIndex - index) <
            Math.abs(best.liveIndex - index)
            ? entry
            : best;
        }, null)?.question?.node || null
      );
    }
    return (
      direct?.node ||
      liveQuestions[Math.min(index, liveQuestions.length - 1)]?.node ||
      null
    );
  }

  function setActiveTimelineQuestion(index, text, fallbackNode) {
    window.__codexConversationTimelineActiveQuestion = {
      index,
      text,
  ...
  function resolveActiveTimelineQuestionNode() {
    const active = window.__codexConversationTimelineActiveQuestion;
    if (!active) return null;
    return (
      resolveTimelineQuestionTarget(active.index, active.text, active.node) ||
      active.node ||
      null
    );
  }
  ...
  function removeConversationTimeline() {
    document
      .querySelectorAll(`.${timelineClass}`)
      .forEach((node) => node.remove());
  }
  ...
  function scheduleTimelineTargetScroll(index, text, fallbackNode) {
    const activationId =
      (window.__codexConversationTimelineActivationId || 0) + 1;
    window.__codexConversationTimelineActivationId = activationId;
    setActiveTimelineQuestion(index, text, fallbackNode);
    const lockedTargetNode =
      resolveTimelineQuestionTarget(index, text, fallbackNode) || fallbackNode;
    const run = (behavior = "smooth", shouldHighlight = false) => {
      if (window.__codexConversationTimelineActivationId !== activationId)
        return;
      if (!scrollTimelineTarget(lockedTargetNode, behavior)) return;
      if (shouldHighlight) highlightTimelineTarget(lockedTargetNode);
    };
    requestAnimationFrame(() => run("smooth", true));
    setTimeout(() => run("auto", false), 80);
    setTimeout(() => run("auto", false), 220);
  }

  function createConversationTimelineMarker(question, questions) {
    const marker = document.createElement("button");
    const markerIndex = questions.indexOf(question);
    const activeNode = resolveActiveTimelineQuestionNode();
    marker.type = "button";
    marker.className = timelineMarkerClass;
    if (activeNode && question.node === activeNode) {
    ...
    const activateMarker = (event) => {
      ...
      if (
        event.type === "click" &&
        now - (marker.__codexConversationTimelineLastActivatedAt || 0) < 300
      ) {
        return;
      }
      marker.__codexConversationTimelineLastActivatedAt = now;
      document
        .querySelectorAll(
          `.${timelineMarkerClass}.codex-conversation-timeline-marker-active`,
        )
      ...
      marker.classList.add("codex-conversation-timeline-marker-active");
      scheduleTimelineTargetScroll(markerIndex, question.text, question.node);
    };
  ...
  function refreshConversationTimeline() {
    if (!codexPlusSettings().conversationTimeline) {
      window.__codexConversationTimelineSnapshot = null;
      window.__codexConversationTimelineActiveQuestion = null;
    ...
    const snapshot = timelineQuestionSnapshot(questions);
    const existingTimeline = document.querySelector(`.${timelineClass}`);
    if (
      existingTimeline?.dataset.codexConversationTimelineVersion ===
        codexConversationTimelineVersion &&
      sameTimelineQuestionSnapshot(
        window.__codexConversationTimelineSnapshot,
        snapshot,
      )
    ) {
      return;
    }
    window.__codexConversationTimelineSnapshot = snapshot;
    removeConversationTimeline();
    const container = document.createElement("div");
    container.className = timelineClass;
    container.dataset.codexConversationTimelineVersion =
      codexConversationTimelineVersion;
    ...
    questions.forEach((question) => {
      container.appendChild(
        createConversationTimelineMarker(question, questions),
      );
    });
  ...
  function runScanStep(step) {
    try {
      ...
      window.__codexSessionDeleteScanFailures =
        window.__codexSessionDeleteScanFailures || [];
      window.__codexSessionDeleteScanFailures.push(
        String(error?.stack || error),
      );
    }
  }
  ...
  function isExtensionUiNode(node) {
    return !!node?.closest?.(
      `.codex-delete-toast, .codex-delete-confirm-overlay, .codex-plus-modal-overlay, .${projectMoveOverlayClass}, .${timelineClass}, .codex-conversation-timeline, #codex-plus-menu`,
    );
  }
  ...
  function isScanRelevantNode(node) {
    ...
    return (
      !!node.matches?.(scanRelevantSelector) ||
      !!node.closest?.(scanRelevantSelector) ||
      !!node.querySelector?.(scanRelevantSelector)
    );
  }
  ...
  function isChatContentMutation(mutation) {
    ...
    if (
      target?.closest?.(
        '[data-message-author-role], [data-testid="conversation-turn"], main .prose',
      )
    ) {
      return false;
    }
    ...
    return mutations.some((mutation) => {
      ...
      return (
        Array.from(mutation.addedNodes).some(
          (node) => node.nodeType === 1 && !isExtensionUiNode(node),
        ) ||
  ...
  window.__codexPlusResizeHandler = () => {
    ...
    codexPlusResizeRafId = requestAnimationFrame(() =>
      updateFloatingCodexPlusMenuPosition(
        document.getElementById(codexPlusMenuId),
      ),
    );
  };
  ...
  window.__codexSessionDeleteObserver.observe(
