...
RequestHandler = Callable[[str, dict[str, object]], dict[str, object] | None]
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        try:
            ...
            if self.path == "/thread-sort-keys":
                ...
                return
            if self.server.request_handler is not None:
                result = self.server.request_handler(self.path, payload)
            ...
            if self.server.request_handler is not None and self.path.startswith("/backend/"):
                self._send_json({"status": "failed", "message": str(exc)}, status=400)
                return
            if self.path == "/export-markdown":
    ...
    def _send_asset(self, name: str) -> None:
        ...
        self._send_json({"error": f"asset not found: {asset_name}"}, status=404)
